    ...
    <div class="max-w-xl m-auto pt-6 pb-6">
        ...
        <form action="{{ route('job-applications.store', ['joblisting' => $job->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-span-full">
                ...
                <div class="mt-2">
                    <input type="number" name="age" id="age" value="{{ old('age') }}" class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                    @error('age')
                        <div class="text-red-950">{{ $message }}</div>
                    @enderror
                </div>
